...
import com.example.angelzheimer2.extras.TextViewUndoRedo;
...
public class AddNotesActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        getSupportActionBar().setTitle(getString(R.string.NovaNota));
...
//            getSupportActionBar().setTitle("Your Note");
    ...
    private void verifyDetails() {
        ...
        if (title.isEmpty() || title.length() < 4) {
            titleField.setError("Deve ser superior a 4 caracteres!");
            titleField.requestFocus();
            return;
        }

        if (note.isEmpty() || note.length() < 4) {
            notesField.setError("Deve ser superior a 4 caracteres!");
        ...
        if (title.equals(tit) && note.equals(not)) {
            Toast.makeText(this, "Sem alterações!", Toast.LENGTH_SHORT).show();

        } else {
            mProgress.setMessage("Submetido");
            ...
            mRef.child(mUser.getUid()).child(key).setValue(map).addOnCompleteListener(new OnCompleteListener<Void>() {
                ...
                public void onComplete(@NonNull Task<Void> task) {
                    if (task.isSuccessful()) {
                        mProgress.dismiss();
                        Toast.makeText(AddNotesActivity.this, "Editado", Toast.LENGTH_SHORT).show();
//                        startActivity(new Intent(AddNotesActivity.this,MainActivity.class));
            ...
            mRef.child(mUser.getUid()).child(time).setValue(map).addOnCompleteListener(new OnCompleteListener<Void>() {
                ...
                public void onComplete(@NonNull Task<Void> task) {
                    if (task.isSuccessful()) {
                        mProgress.dismiss();
                        Toast.makeText(AddNotesActivity.this, "Nova nota adicionada", Toast.LENGTH_SHORT).show();
//                        startActivity(new Intent(AddNotesActivity.this,MainActivity.class));
                        finish();
                    } else {
                        mProgress.dismiss();
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if (item.getItemId() == R.id.up_star) {
            ...
            if (drawable.getConstantState().equals(getResources().getDrawable(R.drawable.ic_star_border_black_24dp).getConstantState())) {
                ...
                Toast.makeText(AddNotesActivity.this, "Marcado como Importante", Toast.LENGTH_SHORT).show();
                ...
                Toast.makeText(AddNotesActivity.this, "Removido de Importante", Toast.LENGTH_SHORT).show();
